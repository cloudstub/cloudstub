...
/**
 * Generates a complete, compilable {@code cloudstub-*} module skeleton from a Smithy service model.
 *
 * <p>All generated response templates are intentional placeholders — they compile and register
 * correctly but return minimal responses. Human review is always required before a generated module
 * is production-ready.
 */
public class ModuleGenerator {
    ...
    private Analysis analyze(Path modelPath) {
        ...
        String pkg = "io.cloudstub." + serviceId;
        String moduleName = "cloudstub-" + serviceId;
    ...
    private Model loadModel(Path modelPath) {
        ...
        if (!notable.isEmpty()) {
            System.err.println(
                    "  "
                            + notable.size()
                            + " model validation note(s) suppressed"
                            + (verbose ? ":" : " — re-run with --verbose for details"));
            if (verbose) {
                notable.forEach(
                        e -> System.err.println("    [" + e.getSeverity() + "] " + e.getMessage()));
            }
        }
        return result.getResult()
                .orElseThrow(() -> new IllegalArgumentException("Model produced no output."));
    }
    ...
    private GeneratedFile buildGradle(String serviceId, String coreVersion) {
        String content =
                """
                dependencies {
                    compileOnly 'io.cloudstub:cloudstub-core:%s'
                    testImplementation 'io.cloudstub:cloudstub-core:%s'
    ...
    private GeneratedFile serviceClass(
            ServiceShape service,
            List<OperationShape> operations,
            Protocol protocol,
            String pkg,
        ...
        String simpleClassName = "CloudStub" + className + "Service";
        ...
        sb.append(
                """
                /**
                 * CloudStub service module for %s.
                 *
                 ...
                 * can parse without error. See existing modules (cloudstub-sqs, cloudstub-secretsmanager)
                 * for examples.
                 */
                """
                        .formatted(className));
        sb.append("public class ")
                ...
                .append(" implements CloudStubService {\n\n");

        sb.append("    private static final String SERVICE_ID = \"")
                .append(deriveServiceId(service))
                .append("\";\n");
        ...
        sb.append("    @Override\n");
        sb.append("    public void register(CloudStubContext context) {\n");
    ...
    private GeneratedFile serviceLoaderFile(String pkg, String className) {
        return new GeneratedFile(
                ...
                pkg + ".CloudStub" + className + "Service\n");
    }
        ...
        op.getOutput()
                .ifPresent(
                        outputId -> {
                            ...
                            sb.append("{{! REVIEW REQUIRED — output: ")
                                    .append(output.getId().getName());
                            if (output instanceof StructureShape struct
                                    && !struct.getAllMembers().isEmpty()) {
                                String members =
                                        struct.getAllMembers().entrySet().stream()
                                                .map(
                                                        e ->
                                                                e.getKey()
                                                                        + ": "
    ...
    private GeneratedFile builderClass(
            Model model, OperationShape op, Protocol protocol, String pkg, boolean isXml) {
        String opName = op.getId().getName();
        ...
        // to an empty builder rather than throwing, mirroring the defensive handling in
        // templateBody.
        ...
        sb.append("/**\n");
        sb.append(" * GENERATED — builds a ")
                .append(isXml ? "XML" : "JSON")
                .append(" response body for {@code ")
                .append(opName)
                .append("}.\n");
        sb.append(" *\n");
        sb.append(
                " * <p>Populate fields from your state store, then call {@link #build()} to get the\n");
        ...
        sb.append(
                "    private final java.util.Map<String, Object> _fields = new java.util.LinkedHashMap<>();\n\n");
                    ...
                    requiredMembers.stream()
                            .map(
                                    m ->
                                            toJavaType(model, m)
                                                    + " "
                                                    + sanitizeIdentifier(m.getMemberName()))
                            .collect(Collectors.joining(", "));
            sb.append("    public ").append(className).append("(").append(params).append(") {\n");
            for (var m : requiredMembers) {
                sb.append("        _fields.put(\"")
                        .append(wireName(m, isXml))
                        .append("\", ")
                        .append(sanitizeIdentifier(m.getMemberName()))
                        .append(");\n");
            }
            sb.append("    }\n\n");
        } else {
        ...
        sb.append("    /** @return ")
                .append(isXml ? "XML" : "JSON")
                .append(" wire-format string for the {@code ")
                .append(opName)
                .append("} response. */\n");
        sb.append("    public String build() {\n");
        if (protocol == Protocol.FORM_URL) {
            sb.append("        return \"<").append(opName).append("Response>\"\n");
            sb.append("                + ResponseSupport.toXml(\"")
                    .append(opName)
                    .append("Result\", _fields)\n");
            sb.append(
                    "                + \"<ResponseMetadata><RequestId>\" + java.util.UUID.randomUUID() + \"</RequestId></ResponseMetadata>\"\n");
            ...
            sb.append("        return ResponseSupport.toXml(\"")
                    .append(xmlRoot)
                    .append("\", _fields);\n");
        } else {
    ...
    /**
     * Maps a Smithy output member to the Java type exposed on the generated builder. Numeric,
     * temporal, and enum shapes get precise types; nested aggregates (structure/union/map/list) are
     * surfaced as generic collections that the shared serialiser understands.
     */
     ...
     * name when present, matching how the real AWS SDK marshals the field. The member name itself
     * is still used for the Java-side builder identifier (see {@link #sanitizeIdentifier(String)}).
     */
     ...
     * {@code [a-zA-Z_][a-zA-Z0-9_]*}, so the only collision with the Java grammar is reserved
     * words; those get a trailing underscore (e.g. {@code default} → {@code default_}). The
     * original member name is still used as the serialised wire key, so this rename never affects
     * the response body.
     */
    ...
    private void appendXmlHelpers(StringBuilder sb) {
        ...
        sb.append(
                "        for (var e : fields.entrySet()) { appendXml(sb, String.valueOf(e.getKey()), e.getValue()); }\n");
        sb.append("        return sb.append(\"</\").append(root).append(\">\").toString();\n");
        sb.append("    }\n\n");
        sb.append(
                "    private static void appendXml(StringBuilder sb, String tag, Object value) {\n");
        sb.append("        if (value instanceof Iterable<?> list) {\n");
        ...
        sb.append(
                "            for (var e : map.entrySet()) { appendXml(sb, String.valueOf(e.getKey()), e.getValue()); }\n");
        sb.append("            sb.append(\"</\").append(tag).append(\">\");\n");
        sb.append("        } else if (value != null) {\n");
        sb.append(
                "            sb.append(\"<\").append(tag).append(\">\").append(xmlEscape(String.valueOf(value))).append(\"</\").append(tag).append(\">\");\n");
        sb.append("        }\n");
        sb.append("    }\n\n");
    ...
    private GeneratedFile testClass(List<OperationShape> operations, String pkg, String className) {
        StringBuilder sb = new StringBuilder();
        sb.append("package ").append(pkg).append(";\n\n");
        sb.append("import io.cloudstub.core.CloudStub;\n");
        ...
        sb.append("class CloudStub").append(className).append("ServiceTest {\n\n");
        sb.append("    static CloudStub cloudMock;\n");
        ...
        sb.append("        cloudMock = new CloudStub().withService(new CloudStub")
                .append(className)
                .append("Service());\n");
    ...
    private String buildJsonTemplate(Model model, StructureShape output) {
        String fields =
                output.getAllMembers().entrySet().stream()
                        .limit(8)
                        .map(
                                e -> {
                ...
                .map(
                        h ->
                                new String[] {
                                    ...
                                    // Greedy labels {key+} span path segments (.+); normal labels
                                    // {id} stay within
                                    // one ([^/]+).
                                    // Replace greedy labels first so the normal-label rule doesn't
                                    // consume them.
                                    h.getUri()
                                            .toString()
                                            .replaceAll("\\{[^}]*\\+}", ".+")
