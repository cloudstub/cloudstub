 ...
 * registered for action {@code Publish} must match a request for {@code Publish} only — never one
 * for {@code PublishBatch} (whose action name has {@code Publish} as a prefix). The old {@code
 * containing("Action=Publish")} matcher over-matched and relied on registration order plus
...
class XmlFormRoutingTest {
    ...
    private static final class PublishOnlyService implements CloudStubService {
        @Override
    ...
    void prefixActionDoesNotMatchLongerAction() throws Exception {
        ...
        assertEquals(
                404, response.statusCode(), "Publish stub must not match a PublishBatch request");
    }
    ...
    void actionMatchedWhenNotFirstParameter() throws Exception {
        ...
        HttpResponse<String> response =
                postForm("TopicArn=arn:aws:sns:us-east-1:000000000000:t&Action=Publish");
