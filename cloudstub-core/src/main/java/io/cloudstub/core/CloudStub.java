...
import io.cloudstub.core.internal.AwsEndpointOverride;
import io.cloudstub.core.internal.CloudStubResponseTransformer;
import io.cloudstub.core.internal.CloudStubSettings;
import io.cloudstub.core.internal.FaultEngine;
import io.cloudstub.core.internal.ModuleInitializer;
import io.cloudstub.core.internal.RequestHistory;
import io.cloudstub.core.internal.ServiceRegistry;
import io.cloudstub.core.internal.WireMockServerFactory;
import io.cloudstub.core.internal.WireMockStubRegistrar;
import io.cloudstub.core.internal.store.StateStoreFactory;
 ...
 * CloudStubService} discovered on the classpath to register their stubs.
 *
 * <p>This class coordinates lifecycle and exposes the public API; the work of each concern is
 * delegated to focused collaborators in {@code io.cloudstub.core.internal}: building the server
 ...
 * </pre>
 */
public final class CloudStub implements AutoCloseable {

    private static final Logger log = LoggerFactory.getLogger(CloudStub.class);
    ...
    private final CloudStubSettings settings = new CloudStubSettings(DEFAULT_MAX_REQUEST_HISTORY);
     ...
     * a CloudStub restart. When not set, an in-memory store is used (state lost on stop).
     *
     * <p>Must be called before {@link #start()}.
     *
     * @param directory directory where the state file is written (a {@code cloudstub-state.log}
     *     append-only log by default; see {@link #withPersistenceBackend})
     * @throws CloudStubAlreadyStartedException if already started
     */
     ...
     * whose {@link CloudStubService#serviceId()} is in {@code serviceIds} are registered; all
     ...
     * before {@link #start()}.
     *
     * @throws CloudStubAlreadyStartedException if the instance is already started
     */
     ...
     *
     * @throws CloudStubAlreadyStartedException if this instance is already started
     */
    public void start() {
        requireNotStarted();
        ...
        CloudStubResponseTransformer transformer =
                new CloudStubResponseTransformer(stateStore, registry, faultEngine);
        ...
        log.info("CloudStub started on port {}", server.port());
    }
     ...
     *
     * @throws CloudStubNotStartedException if not yet started
     */
     ...
     * active.
     *
     * <p>Called automatically by {@code CloudStubExtension} after each test method.
     */
    ...
    private void requireStarted() {
        if (server == null) {
            throw new CloudStubNotStartedException();
        }
    }

    private void requireNotStarted() {
        if (server != null) {
            throw new CloudStubAlreadyStartedException();
        }
    }
}
