...
│                                                       │
│  cloudstub-sqs   cloudstub-s3   cloudstub-secrets  …  │
...
The AWS SDK is redirected to the local CloudStub port via system property injection (`aws.endpoint-url`). No application
...
- Running `ServiceLoader.load(CloudStubService.class)` to discover and initialise all installed modules.
- Providing the Junit 6 `@ExtendWith(CloudStubExtension.class)` lifecycle hook so tests require no manual setup.
...
The `StubRegistrar` is a CloudStub-native facade. It exposes stub registration without leaking WireMock types:
...
`cloudstub-core`; modules declare a minimum core version via a manifest attribute.
...
CloudStub's clean SPI contract makes it a natural target for an agent layer. Four capabilities are planned:

### Stub generation agent

Takes an AWS Smithy or OpenAPI service model as input and auto-generates a complete CloudStub module: path matchers,
...
pairs, and synthesises CloudStub stubs from them. Particularly valuable for complex services like Step Functions and
EventBridge where hand-authoring templates is error-prone.

### Test assertion agent

Given a test run against CloudStub, the agent inspects the interaction log and flags correctness issues: a consumer that
never called `DeleteMessage` after processing, a response field asserted by the test but never populated by the stub,
and so on. Turns CloudStub from a passive stub server into an active correctness oracle.
...
mid-flight, RDS timeout after successful dequeue) and produces the corresponding CloudStub fault-injection stubs. This
automates the resilience test coverage that engineers rarely write by hand.

The agent layer sits above `cloudstub-core` and interacts exclusively via the `StubRegistrar` SPI. It has no dependency
...
- CloudStub is not a full reimplementation of AWS. Complex service behaviours (SQS FIFO deduplication, S3 multipart
...
Rejected due to container overhead, Docker dependency, and online license requirements for the Pro tier. CloudStub is
...
**Resolved:** Module JARs declare the minimum compatible `cloudstub-core` version using the `CloudStub-Core-Min-Version`
...
reads it with `getClass().getModule()` / `JarFile` at runtime. `cloudstub-core` itself follows semantic versioning;
any change to the `StubRegistrar` or `CloudStubService` interface constitutes a breaking change and requires a major
version bump.

Module authors should set `CloudStub-Core-Min-Version` to the oldest core release that contains all `StubRegistrar`
