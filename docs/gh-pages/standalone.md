...
Either way you get one self-contained JAR — the CloudStub engine (with shaded WireMock/Jetty) and the
launcher, and nothing else. Service modules ship as separate jars, fetched on demand (see below). The
examples below use the build path `cloudstub-local/build/libs/cloudstub-local.jar`; if you downloaded
the JAR, run `cloudstub-local.jar` instead.
