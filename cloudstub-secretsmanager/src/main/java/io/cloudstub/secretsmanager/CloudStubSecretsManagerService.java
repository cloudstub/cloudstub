 ...
 *
 * <p>Registers stateless JSON/X-Amz-Target stubs for the core Secrets Manager operations. Requests
 * arrive as {@code POST /} with header {@code X-Amz-Target: secretsmanager.<Operation>}. Responses
 * are well-formed JSON that {@code SecretsManagerClient} (AWS SDK v2) parses without error. Secret
 * state is not simulated — {@code GetSecretValue} always returns a fixed {@code SecretString}
 * regardless of what was passed to {@code CreateSecret}.
 *
...
public class CloudStubSecretsManagerService implements CloudStubService {
    ...
    private static final String LIST_SECRETS =
            """
            {"SecretList":[{"ARN":"%scloudstub-secret","Name":"cloudstub-secret"}]}"""
                    .formatted(ARN_PREFIX);
