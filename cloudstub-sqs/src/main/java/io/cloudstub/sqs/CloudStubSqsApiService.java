 ...
 * same {@link SqsKeys} scheme) as the AWS-protocol stubs in {@link CloudStubSqsService}. So a
 * message sent through the AWS SDK is returned by {@code GET /api/sqs/receive-message} and shown in
 * the console, and vice versa — one state, two representations (AWS wire protocol vs. this friendly
 * JSON).
 *
 * <p>Discovered via {@code META-INF/services/io.cloudstub.core.spi.CloudStubApiService}.
 */
public class CloudStubSqsApiService implements CloudStubApiService {
    ...
    public void registerRoutes(CloudStubApiContext context) {
        ...
        r.register(
                ...
                List.of(QUEUE, new ApiParam("body", true, "Message body")),
                this::sendMessage);
        r.register(
                HttpMethod.GET,
                "/receive-message",
    ...
    private ApiResponse purgeQueue(ApiRequest req) {
        ...
        return new ApiResponse(200, Map.of("status", "purged", "queue", queue));
    }
}
